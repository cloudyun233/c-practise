...
if __name__ == "__main__":
    ...
    try:
        ...
        print("请先运行 01_data_preprocessing.py 生成训练数据")
        exit(1)
    ...
    # ==================== 绘制收敛曲线图 ====================
    print("[INFO] 正在绘制贝叶斯优化收敛曲线...")
    
    # 获取每次迭代的评估值
    func_vals = res.func_vals
    iterations = range(1, len(func_vals) + 1)
    ...
    # 绘制每次迭代的实际评估值（橙色散点）
    ax.scatter(iterations, func_vals, color='orange', s=30, alpha=0.6, 
               label='Actual Evaluation', zorder=2)
    
    # 绘制当前最优值曲线（蓝色实线）
    ax.plot(iterations, cumulative_min, 'b-', linewidth=2, 
            label='Best So Far (Cumulative Min)', zorder=3)
    
    # 标注最终最优值
    ax.axhline(y=res.fun, color='red', linestyle='--', linewidth=1, 
               alpha=0.7, label=f'Final Best: {res.fun:.4f}')
    
    # 设置标签和标题
    ax.set_xlabel('Iteration Number', fontsize=12)
    ax.set_ylabel('CV-RMSE', fontsize=12)
    ax.set_title('Bayesian Optimization Convergence Process', fontsize=14)
    ax.legend(loc='upper right', fontsize=10)
    ax.grid(True, alpha=0.3, linestyle='--')
    
    # 设置y轴范围（留出一些边距）
    y_min = min(func_vals) * 0.98
    y_max = max(func_vals) * 1.02
    ...
    print("[INFO] 收敛曲线图已保存至 bayesian_convergence.png")